//Congshu Wang revised on Nov 9, 2014
...
tokenClass scannerClass::getToken()
{
	...
	do
	{
		...
		if (state == 100)
		{
			...
			else if (_stricmp(theLex.c_str(), "invert") == 0)
			{
				token.setToken("invert", INVERT_T, NONE);
				state = 0;
				theLex = "";
			}
		...
		else if (state == 127)
		{
			...
			else if (theLex[2] == 's') //it is {$s+}
			{
				if (theLex[3] == '+')
				{
					cout<<theSymbolTable.print()<<endl;
				}
			}
			else  //warning if it is not lower case p or lower case e or lower case s
			{
...
void tokenClass::display()
{
	...
	/*---------------------------*/
	else if (tokentype == INVERT_T)
		printOutString += " INVERT_T";
	/*---------------------------*/
