...
void parserClass::parse_stmt()
{
	...
	else if (token.getTokenType() == INVERT_T)
	{
		token = theScanner.getToken();
		parse_flInvert();
	}
	else
	{
		...
		errorexit("one of those keywords: [while, if, cout, cin, begin, invert] / an identifier", token.getLexeme()); 
	}
}

void parserClass::parse_flInvert()
{
	if (token.getTokenType() == IDEN_T) //must follow an ident
	{
		flInvertIdLookUpResult = theScanner.theSymbolTable.tableLookUp(token.getLexeme());
		if (flInvertIdLookUpResult == NULL)
			undeclaredErrorExit(token.getLexeme());
		else
		{
			//can only follow id in int or boolean types
			if (flInvertIdLookUpResult->symbolType == FUNC_ID_TYPE)
			{
				cout << "Error: Type mismatch since identifier after keyword Invert must be integer type or boolean type." << endl;
				theScanner.printCurrentLine();
				cin.get();
				exit(1);
			}
			else // ident is int or boolean type
			{
				if (flInvertIdLookUpResult->symbolType == INT_TYPE)
				{
					SymbolNode *invertNode = addTempVar(INT_TYPE);
					printInstruction("movw", NULL, intToPali(-1), invertNode, "");
					SymbolNode *inverResult = addTempVar(INT_TYPE);
					printInstruction("movw", flInvertIdLookUpResult, "", inverResult, "");
					printInstruction("mulw", invertNode, "", inverResult, "");
					printInstruction("movw", inverResult, "", flInvertIdLookUpResult, "");
					printInstruction("movw", NULL, "r1", NULL, "sp");
					printInstruction("addw", NULL, intToPali(4), NULL, "sp");
					theScanner.theSymbolTable.top->deleteTempSymbol();
				}
				else // when it's boolean type
				{
					string falseToTure = getNextTempVar();
					string afterFalseToTrue = getNextTempVar();
					//printInstruction("cmpw", NULL, intToPali(0), NULL, "@r1"); //compare result with false
					printInstruction("cmpw", NULL, intToPali(0), flInvertIdLookUpResult, ""); //compare result with false
					//printInstruction("movw", NULL, "r1", NULL, "sp");//subw #4 sp
					printInstruction("beq", NULL, falseToTure);
					//invert from true to false
					printInstruction("movw", NULL, intToPali(0), flInvertIdLookUpResult, "");
					printInstruction("jmp", NULL, afterFalseToTrue);
					printBranch(falseToTure);
					//invert from false to true
					printInstruction("movw", NULL, intToPali(1), flInvertIdLookUpResult, "");
					printBranch(afterFalseToTrue);
	...
	{
		cout << "Error: Invert keyword must be followed by a single identifier." << endl;
		theScanner.printCurrentLine();
		cin.get();
		exit(1);
	}
...
void parserClass::parse_flBegin()
{
	...
	else if (token.getTokenType() == WHILE_T || token.getTokenType() == IF_T || token.getTokenType() == IDEN_T || token.getTokenType() == COUT_T || token.getTokenType() == CIN_T || token.getTokenType() == BEGIN_T || token.getTokenType() == INVERT_T)
	{
		parse_stmt();
		parse_stmtail();
		if (token.getTokenType() == END_T)
		...
		errorexit("one of those keyword [end, while, if, cout, cin, begin, invert] / an identifier", token.getLexeme());
}
...
void parserClass::parse_stmtail()
{
	if (token.getTokenType() == SEMICOLN_T)
	{
		...
		errorexit("a single identifier after keyword Invert / a semicolon to seperate multiple statements / a keyword [end] after last statement", token.getLexeme());
}
...
{
	parse_varDecs();
	//after reading all the global vars
