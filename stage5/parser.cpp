//Congshu Wang revised on Nov 10, 2014
		...
		else
		{
			...
			else if (checkPrecedence(topTerm, token.getTokenType()) == GT)
			{
				...
				do
				{
					...
					tempArrayLexeme[counter] = theStack.returnLastPoppedTokenLexeme();
					++counter;
				...
				if (isValidRHS(tempArrayType, tempArraySubType, tempArrayLexeme, counter))
				{
...
bool parserClass::isValidRHS(int tempArrayType[], int tempArraySubType[], string tempArrayLexeme[], int counter)
{
	SymbolNode *result;
	if (counter == 1)
	{
		...
		{
		case IDEN_T:
			result = theScanner.theSymbolTable.tableLookUp(tempArrayLexeme[0]);
			if (result == NULL)
...
void parserClass::parse_stmt()
{
	...
	else if (token.getTokenType() == IDEN_T)
	{
		...
		if (result != NULL)
		{
			if (result->symbolType == FUNC_ID_TYPE)
			{
				numOfFuncParams = result->numOfParams;
			}
		}
		else
...
void parserClass::parse_flId()
{
	...
	else if (token.getTokenType() == OPENPARENT_T)
	{
		...
		{
			if (funcParamsCount != numOfFuncParams)
			{
				cout << "Error: Incorrect number of parameter(s).  Expecting " << numOfFuncParams << " parameter(s) but found " << funcParamsCount << "." << endl;
				numOfFuncParams = NULL;
				funcParamsCount = NULL;
				theScanner.printCurrentLine();
				cin.get();
				exit(1);
			}
	...
	else if (token.getTokenType() == SEMICOLN_T || token.getTokenType() == END_T)
	{
		...
		if (numOfFuncParams != 0)
		{
			//if the function has parameters required but found nothing
			cout << "Error: Incorrect number of parameter(s).  Expecting " << numOfFuncParams << " parameter(s) but found none." << endl;
			numOfFuncParams = NULL;
			theScanner.printCurrentLine();
			cin.get();
			exit(1);
...
void parserClass::parse_funDecs()
{
	if (token.getTokenType() == FUNCTION_T)
	{
		...
		if (token.getTokenType() == IDEN_T)
		{
			if (!theScanner.theSymbolTable.tableAddEntry(new SymbolNode(token.getLexeme(), FUNC_ID, FUNC_ID_TYPE)))
				dupErrorExit(token.getLexeme());
